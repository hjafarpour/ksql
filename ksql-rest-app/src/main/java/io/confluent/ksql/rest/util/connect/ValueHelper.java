  ...
  static Object decimal(Schema schema, Object value) {
    ...
    if (value instanceof BigDecimal) {
      ...
      if (scale == decimal.scale()) {
        ...
        return decimal.setScale(scale, DEFAULT_SCALE);
      }
    }
      ...
      case STRUCT:
        if (value instanceof Struct) {
          ...
          if (map.containsKey("schema") && map.get("fieldValues") instanceof List) {
            ...
            result = ObjectMapperFactory.INSTANCE.convertValue(value, Struct.class);
          } else {
            LOG.trace("map");
            Struct struct = new Struct(schema);
            for (Map.Entry<?, ?> kvp : map.entrySet()) {
              LOG.trace("field {}", kvp.getKey());
              Field field = schema.field((String) kvp.getKey());
